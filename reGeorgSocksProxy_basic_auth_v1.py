...
class session(Thread):
    def __init__(self, pSocket, connectString,BasicAuthentication):
		Thread.__init__(self)
		self.pSocket = pSocket
		self.connectString = connectString
		self.BasicAuthentication = base64.standard_b64encode(BasicAuthentication)
		#b64auth = base64.standard_b64encode(BasicAuthentication)
    ...
    def setupRemoteSession(self, target, port):
        ...
        headers = {"X-CMD": "CONNECT", "X-TARGET": target, "X-PORT": port,"Authorization": "Basic %s" % self.BasicAuthentication}
        self.target = target
        self.port = port
        cookie = None
    ...
    def closeRemoteSession(self):
        headers = {"X-CMD": "DISCONNECT", "Cookie": self.cookie,"Authorization": "Basic %s" % self.BasicAuthentication}
        params = ""
        conn = self.httpScheme(host=self.httpHost, port=self.httpPort)
        ...
        while True:
            try:
                if not self.pSocket:
                    break
                data = ""				
                headers = {"X-CMD": "READ", "Cookie": self.cookie, "Connection": "Keep-Alive","Authorization": "Basic %s" % self.BasicAuthentication}
    ...
    def writer(self):
        ...
        while True:
            try:
                ...
                if not data:
                    break
                headers = {"X-CMD": "FORWARD", "Cookie": self.cookie, "Content-Type": "application/octet-stream", "Connection": "Keep-Alive","Authorization": "Basic %s" % self.BasicAuthentication}
...
def askGeorg(connectString,BasicAuthentication=""):
    ...
    if not BasicAuthentication:
		...
		headers = urllib3.util.make_headers(basic_auth=BasicAuthentication)
...
if __name__ == '__main__':
 ...
 |__|\__\|______||______|  __||______|\_____/|__|\__\|______|
                    |_____|
                    ... Basic Authetication version 2018.6.22 by Dio
  ...
  \033[0m
   """
	...
	parser.add_argument("-b", "--basic-auth", metavar="", help="basic-auth use format 'username:password'", default="")
	...
	if not askGeorg(args.url,args.basic_auth):
	...
	while True:
		try:
			...
			session(sock, args.url,args.basic_auth).start()
